...
function TouchpointQuestionary(props: TouchpointQuestionaryProps) {
  ...
  function changeTouchPointStatus(e: any, index: number) {
    let tempt = props.TouchPoints;
    tempt[index].Status = e.target.value;
    props.updateTouhcPoints(_.cloneDeep(tempt));
  }

  function changeTouchpointDevation(e:any, index:number){
    let tempt = props.TouchPoints;
            ...
            <Grid style={{
              ...
              {x.receiver != undefined && <Grid item xs={6}><FormGroup>
                <FormControlLabel labelPlacement="top" control={<Checkbox
                  ...
                  onChange={(e: any) => changeTouchpointDevation(e,index)
                  } />
                  } label={"Devation"} /> 
                  </FormGroup>
                  </Grid>}

              {x.receiver != undefined && x.devation &&
                <Grid item xs={6}>
                  <FormControl fullWidth>
                    ...
                    <Select
                      label="Status"
                      value={x.Status} 
                      onChange={(e) => changeTouchPointStatus(e, index)}
                    >
