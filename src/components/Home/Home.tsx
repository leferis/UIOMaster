...
function Home(props: HomeProps) {
  ...
  return (<Group>
    {props.journeys.filter((x) => {
      ...
      if (maxXPlanned < journey.Toucpoint.length * 200) { maxXPlanned = journey.Toucpoint.length * 200; }
    ...
    {props.journeys.filter((x) => {
      ...
      return (<JourneyVisualization CloseHomeWindow={props.CloseHomeWindow} setJourney={props.setJourney} journeyID={getJourneyIndex(journey)} IncludeActor={true} ActorImage={getMainActor(journey.Actors)} Toucpoint={journey.Toucpoint} journeyIndex={index} getImageObject={props.getImageObject} XINITIALPOSITION={maxXPlanned + 300}></JourneyVisualization>)
    })}
  </Group>)
}
export default Home;
